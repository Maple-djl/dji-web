  ...
  <div class="drone-control-wrapper">
    ...
    <div class="box">
      ...
      <div class="row">
        ...
        <Button size="small" ghost @click="onDrcCameraModeSwitch">
          <span>DRC Camera Mode</span>
        </Button>
      </div>
      <div class="row">
  ...
  try {
    ...
    if (code === 0) {
      ...
      if (data.sub && data.sub.length > 0) {
        ...
        console.log('📡 订阅主题 (subTopic):', deviceTopicInfo.subTopic)
      }
      if (data.pub && data.pub.length > 0) {
        deviceTopicInfo.pubTopic = data.pub[0]
        console.log('📤 发布主题 (pubTopic):', deviceTopicInfo.pubTopic)
      }
      console.log('🔗 完整Topic信息:', {
        subTopic: deviceTopicInfo.subTopic,
        pubTopic: deviceTopicInfo.pubTopic,
        deviceSn: props.sn
      })
      // 获取飞行控制权
      if (droneControlSource.value !== ControlSource.A) {
        console.log('🔑 正在获取飞行控制权...')
        const authResult = await postFlightAuth(props.sn)
        console.log('🔑 飞行控制权获取结果:', authResult)
      }
      console.log('🎮 当前控制源:', droneControlSource.value)
...
// DRC相机模式切换
async function onDrcCameraModeSwitch () {
  if (!flightController.value) {
    message.error('请先获取飞行控制权')
    return
  }
  if (!payloadSelectInfo.payloadIndex) {
    message.error('请先选择负载')
    return
  }

  const currentCameraMode = payloadSelectInfo.camera?.camera_mode
  const newCameraMode = currentCameraMode === CameraMode.Photo ? CameraMode.Video : CameraMode.Photo

  // 使用DRC方式发送相机模式切换指令
  handleDrcCameraModeSwitch(payloadSelectInfo.payloadIndex, newCameraMode)

  message.success(`DRC相机模式切换: ${newCameraMode === CameraMode.Photo ? '拍照' : '录像'}`)
}
